...
// including the opt-out marker handshake with the app's automatic
// registration.
func withServiceManager(t *testing.T, goos string, run service.Runner) *service.Manager {
	...
	m := &service.Manager{
		...
		OptOutFile: service.OptOutPath(filepath.Join(t.TempDir(), "cfg")),
	}
...
func TestServiceInstallLinux(t *testing.T) {
	...
	// A previous uninstall's opt-out marker: install must clear it,
	// re-arming the app's automatic registration.
	...
	require.NoFileExists(t, m.OptOutFile, "install clears the opt-out marker")
...
func TestServiceInstallRefusesForeignOwner(t *testing.T) {
	m := withServiceManager(t, "linux", systemctlScript(t, nil, nil))
	sys := t.TempDir()
	m.SystemUnitDirs = []string{sys}
	require.NoError(t, os.WriteFile(filepath.Join(sys, "competent-search-thing.service"), []byte("deb"), 0o644))
	gui := &guiRecorder{}

	code, _, stderr := run(t, gui, "service", "install")
	require.Equal(t, 1, code)
	require.Contains(t, stderr, "deb-installed unit")
	require.Contains(t, stderr, "exactly one startup manager")
}
...
func TestServiceUninstallNothingInstalled(t *testing.T) {
	...
	require.Contains(t, stdout, "auto-registration disabled")
	require.FileExists(t, m.OptOutFile, "the opt-out is recorded even with nothing installed")
}

func TestServiceUninstallRemovesAndOptsOut(t *testing.T) {
	m := withServiceManager(t, "linux", systemctlScript(t, map[string]string{
	...
	require.Contains(t, stdout, "auto-registration disabled")
	require.Contains(t, stdout, "service install", "the undo path is named")
	require.NoFileExists(t, unitPath)
	require.FileExists(t, m.OptOutFile, "uninstall records the opt-out the app's auto-hook respects")
}

func TestServiceUninstallNoConfigDirNotesMarkerFailure(t *testing.T) {
	m := withServiceManager(t, "linux", systemctlScript(t, nil, nil))
	m.OptOutFile = ""
	gui := &guiRecorder{}

	code, stdout, _ := run(t, gui, "service", "uninstall")
	require.Equal(t, 0, code, "the uninstall itself still succeeds")
	require.Contains(t, stdout, "note: recording the opt-out marker:")
}

...
// a standalone config read like WindowTranslucent. An explicit
// preview.enabled=false -- the opt-out; the pane is ON by default
// since rootsVersion 8 -- or any config error means the configured
// base size
...
func PreviewWindowSize() (w, h int, enabled bool) {
	cfg, err := config.Load()
	if err != nil || !config.Enabled(cfg.Preview.Enabled) {

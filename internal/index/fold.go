...
// blob, or -1. pat must be non-empty pre-folded ASCII without NUL
// (foldPattern output after the caller's NUL check). This is the
// engine's hot loop: it scans for the rarest pattern byte's two case
// variants with bytes.IndexByte (taking the nearer hit) and verifies
// the full pattern around each candidate with a fold-compare.
func ciIndexASCII(blob []byte, pat string) int {
	...
	c1 := pat[a]
	c2 := upperVariant(c1)
	// The anchor can sit no earlier than a (candidate start >= 0) and
	// no later than hi (candidate must fit before the blob end).
	hi := len(blob) - m + a
	n1 := nextIndexByte(blob, c1, a, hi)
	n2 := -1
	if c2 != c1 {
		n2 = nextIndexByte(blob, c2, a, hi)
	}
	for {
		h := n1
		if h < 0 || (n2 >= 0 && n2 < h) {
		...
		if start := h - a; ciMatchAround(blob, start, pat, a) {
			return start
		}
		if h == n1 {
			n1 = nextIndexByte(blob, c1, h+1, hi)
		} else {
			n2 = nextIndexByte(blob, c2, h+1, hi)
		}
	}
}

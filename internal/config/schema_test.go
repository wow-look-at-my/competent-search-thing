...
func TestDefaultConfigMatchesSchema(t *testing.T) {
	...
	full := Config{
		...
		Search: SearchConfig{
			...
			Telemetry: TelemetryConfig{MaxSizeKB: 1024},
			Arbiter:   ArbiterConfig{Enabled: Bool(false)},
		},
		...
		Plugins: PluginsConfig{
			...
			Entries: map[string]PluginEntry{
				"calc": {Enabled: Bool(false), Settings: json.RawMessage(`{"precision":4}`)},
			},
		},
		Bangs: BangsConfig{
			Sigils:  []string{"!", "/", "@"},
		...
		Tray:    TrayConfig{Enabled: Bool(false)},
		History: HistoryConfig{PersistEnabled: Bool(false)},
		Stats:   StatsConfig{Enabled: Bool(false)},
...
func TestConfigSchemaRejectsInvalid(t *testing.T) {
	...
	cases := map[string]string{
		...
		"search fuzzy typo":               `{"search":{"fuzzyEnabld":true}}`,
		"non-bool search fuzzyEnabled":    `{"search":{"fuzzyEnabled":"yes"}}`,
		"old search fuzzyDisabled key":    `{"search":{"fuzzyDisabled":false}}`,
		"non-integer maxWatches":          `{"watcher":{"maxWatches":"lots"}}`,
		"negative sweepMinutes":           `{"watcher":{"sweepMinutes":-5}}`,
		"watcher sweep typo":              `{"watcher":{"sweepEnabld":true}}`,
		"non-bool sweepEnabled":           `{"watcher":{"sweepEnabled":"yes"}}`,
		"old watcher sweepDisabled key":   `{"watcher":{"sweepDisabled":false}}`,
		...
		"old priors disabled key":         `{"search":{"priors":{"disabled":true}}}`,
		"unknown priors key":              `{"search":{"priors":{"enabled":false,"halfLifeDays":7}}}`,
		...
		"old arbiter disabled key":        `{"search":{"arbiter":{"disabled":true}}}`,
		"unknown arbiter key":             `{"search":{"arbiter":{"enabled":false,"minPicks":10}}}`,
		"frecency key typo":               `{"search":{"frecency":{"halfLifeDay":7}}}`,
		"non-bool frecency enabled":       `{"search":{"frecency":{"enabled":"yes"}}}`,
		"old frecency disabled key":       `{"search":{"frecency":{"disabled":true}}}`,
		...
		"removed retainQueries key":       `{"search":{"telemetry":{"retainQueries":true}}}`,
		...
		"old plugins disabled key":        `{"plugins":{"disabled":true}}`,
		"old plugin entry disabled key":   `{"plugins":{"entries":{"calc":{"disabled":true}}}}`,
		"unknown top-level typo":          `{"maxResluts":50}`,
		"tray enabled typo":               `{"tray":{"enabld":true}}`,
		"non-bool tray enabled":           `{"tray":{"enabled":"yes"}}`,
		"old tray disabled key":           `{"tray":{"disabled":true}}`,
		"history persist typo":            `{"history":{"persistEnabld":true}}`,
		"non-bool history persistEnabled": `{"history":{"persistEnabled":"yes"}}`,
		"old history persistDisabled key": `{"history":{"persistDisabled":true}}`,
		"stats enabled typo":              `{"stats":{"enabld":true}}`,
		"non-bool stats enabled":          `{"stats":{"enabled":"yes"}}`,
		"old stats disabled key":          `{"stats":{"disabled":true}}`,
		"old rewrite disabled key":        `{"rewrites":[{"name":"n","pattern":"p","replacement":"r","disabled":true}]}`,
		"non-bool rewrite enabled":        `{"rewrites":[{"name":"n","pattern":"p","replacement":"r","enabled":"yes"}]}`,

...
//
// Two entry surfaces share the Manager: the CLI's explicit
// install/uninstall/status/restart commands, and Ensure (ensure.go),
// the app's automatic first-run registration -- the decision matrix
// the GUI runs at every startup, which never starts anything and
// yields to foreign owners (brew services, the deb-shipped unit) and
// to the user's opt-out marker.
//
...
type Manager struct {
	...
	// OptOutFile is the auto-registration opt-out marker path
	// (<configDir>/service.optout; see OptOutPath). Empty = no marker
	// checks or writes -- the config dir could not be resolved.
	OptOutFile string
	// SystemUnitDirs are the system-level systemd user-unit
	// directories probed for a deb-shipped unit (linux foreign-owner
	// detection). Nil = no deb probe (bare test Managers stay
	// hermetic; NewManager fills the real directories).
...
func NewManager() (*Manager, error) {
	...
	optout := ""
	if dir, cfgErr := config.Dir(); cfgErr == nil {
		optout = OptOutPath(dir)
	}
	return &Manager{
		GOOS:       runtime.GOOS,
		Run:        Run,
		...
		Getenv:     os.Getenv,
		OptOutFile: optout,
		// /lib is a /usr/lib symlink on merged-usr systems; probing
		// both spellings costs one extra stat and covers the rest.
		SystemUnitDirs: []string{"/usr/lib/systemd/user", "/lib/systemd/user"},
	}, nil
}
...
// fileExists reports a plain stat hit (any file kind).

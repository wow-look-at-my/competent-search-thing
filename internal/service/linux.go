...
// A foreign owner (the deb-shipped system unit, a brew services user
// unit) refuses the install: a user unit here would shadow the deb's
// system unit, and exactly one manager may own login startup (the
// darwin brewServiceOwner rationale; see ensure.go).
func (m *Manager) installLinux(ctx context.Context) (InstallResult, error) {
	...
	if owner, hint, owned := m.linuxForeignOwner(); owned {
		msg := fmt.Sprintf("%s already manages %s login startup; exactly one startup manager may own the app", owner, appName)
		if hint != "" {
			msg += " -- " + hint
		}
...
func (m *Manager) statusLinux(ctx context.Context) (StatusInfo, error) {
	...
	if owner, _, owned := m.linuxForeignOwner(); owned {
		st.Extra = append(st.Extra, owner+" also manages the app; exactly one startup manager should own it")
	}

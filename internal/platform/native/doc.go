...
// hotkey registration, cursor and monitor queries, and (macOS only)
// native window moves. Package platform holds every piece of logic
// that can be pure; this package only translates between those pure
// types and the operating system.
//
...
//     fails and callers fall back: no global hotkey, and the window
...
//     user32 syscalls for cursor/monitors. Compile-checked only on
//     Windows; CI builds linux/amd64.
//   - macOS: golang.design/x/hotkey (CGEventTap; needs the
//     Accessibility permission, Register errors without it) plus a
//     small Cocoa cgo shim for cursor/screens/window moves. The shim
//     converts between Cocoa's bottom-left-origin global coordinates
//     and the top-left-origin virtual desktop used everywhere else.

...
- `plugins` -- the [plugin system](#plugins). `disabled` (default
  ...
  `app`, `apps` and `firefox-frequent` work here too); `settings` is an
  opaque JSON object passed verbatim to that plugin in every request
  (its `settings` field), so plugins can be configured without editing
  their manifest.
...
`bangs`, `app`, `apps`, `firefox-frequent`):
...
The fourth built-in, `firefox-frequent`, has no bang: it answers plain

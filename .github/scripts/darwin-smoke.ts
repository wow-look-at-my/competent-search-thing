...
// legacy v1 line protocol still answers byte-for-byte (a8-legacy-ipc), and
// (e) once the big build completes, the watch layer comes up on the FSEvents
// whole-filesystem backend (B-backend) with the process's open-fd count far
// below kern.maxfilesperproc (B-fd-headroom) -- the regression pins for the
// macOS field incident where unbounded per-directory kqueue watches pinned
// the process at its fd ceiling and broke every later open()/exec.
//
